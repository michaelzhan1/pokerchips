...
export default function Home() {
  ...
  const joinRoom = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const name = (e.currentTarget as HTMLFormElement).nameField.value;
    const res = await fetch(process.env.NEXT_PUBLIC_SERVER_URL + `/api/checkRoomId/${roomId}`);
    const data = await res.text();
    if (data === 'true') {
      router.push(`${roomId}?amt=${amt}&name=${name}`);
    } else {
      ...
      <div className='w-screen flex flex-col items-center'>
        ...
        <div className='w-80 mt-4'>
          <form onSubmit={joinRoom}>
            ...
            <input type='text' name='nameField' placeholder='Enter Name' className='w-full border border-gray-300 rounded p-2' />
